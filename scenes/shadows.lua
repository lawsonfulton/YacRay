...
checker = gr.material({0.2, 0.6, 0.9}, {0.1, 0.1, 0.1}, 0)


require('readobj')
mesh = gr.obj_mesh('test_mesh','meshes/grate.obj')
--mesh = gr.mesh('ball', readobj('meshes/cow.obj'))
mesh:translate(0.35,1.5,0)
mesh:scale(3,3,3)
--mesh:rotate('y',180)
mesh:set_material(checker)
...
-- lights
light_color = {0.780131 * 1.3, 0.780409 * 1.3, 0.775833 * 1.3}
...
sqlight = gr.rect_light({0, box_height - 2.01, -2}, 3, 3, light_color, {1,0,0}, 100)
...
-- 	  {0.2, 0.2, 0.2}, {light1, light2})
gr.render(scene,
	  'shadows.png', 700, 700, 4,
	  {0, 5, -13}, {0, 3.5, 0}, {0, 1, 0}, 50,
	  {0.2,0.2,0.2}, {sqlight})
