...
-- -- --pointer
-- pointerMat = gr.fancy_material({0.1, 0.1, 0.1}, {1, 1, 1}, 20000000000.0, 0.8,1.0,0.0,1)
...
monkeyMat = gr.fancy_material({0.8, 0.8, 0.8}, {0, 0, 0}, 20000000000.0, 0.8,1.0,0.0,1)

require('readobj')
monkey = gr.obj_mesh("monkey", "meshes/monkey/monkey-0-subv.obj")
--monkey = gr.mesh('cow', readobj("meshes/monkey/monkey-4-subv.obj"))
-- monkey:scale(0.05,0.05,0.05)
-- monkey:translate(-0.5, 0, 0.5) 
-- monkey:rotate('y', 150)
monkey:set_material(monkeyMat)
...
-- lights
light_color = {1,1,1}
sqlight = gr.rect_light({5, 7, 3}, 3, 3, light_color, {1,0,0}, 1)
...
gr.render(scene,
	  'test.png', 200, 200, 1,
	  1, 0, 0, --dof: samples, aperature, focal len
	  false, 1.0, 0.18, --tone mapping
	  {1/zoom, 1.8/zoom, 3/zoom}, {0, 1, 0}, {0, 1, 0}, 50,
